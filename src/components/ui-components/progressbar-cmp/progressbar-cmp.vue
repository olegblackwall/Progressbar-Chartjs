<template>
    <div class="progressbar">
        <div class="progressbar__container">
            <svg progressbar__container__circles :height="radius * 2" :width="radius * 2">
                <circle
...
const transform = computed(() => {
    return props.type === 'arc' ? `rotate(135, ${props.radius}, ${props.radius})` : `rotate(-90, ${props.radius}, ${props.radius})`
})
...
const current_color = computed(() => `hsl(${(props.percent / 100) * 120}, 100%, 50%)`)

const computed_status = computed(() => {
    if (props.percent === 100) return 'success'
    if (props.percent >= 60) return 'in progress'
    if (props.percent >= 30) return 'warning'
    return 'error'
})
</script>
